...
echo "=========================================="
echo "  NutriAI Beta Server Setup"
echo "  Simple Database Approach (No Google)"
echo "=========================================="
echo ""

cd /root/NutriWeb/server

# Backup the old index.js
if [ -f "index.js" ]; then
    echo "📦 Backing up old server file..."
    cp index.js index.js.backup
fi

# Create the new simplified server code
echo "📝 Creating simplified server code..."
...
async function sendWelcomeEmail(email, platform) {
  ...
  try {
    ...
    const androidLink = process.env.ANDROID_BETA_LINK || 'https://play.google.com/apps/testing/com.nutriai.app';
    const iosLink = process.env.IOS_BETA_LINK || 'https://testflight.apple.com/join/YOURCODE';
...
echo ""
echo "Next steps:"
echo "1. Configure your .env file with Hostinger email settings"
echo "2. Install dependencies: cd /root/NutriWeb/server && npm install"
